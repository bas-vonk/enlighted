<template>
    <base-card :observedAtTimestamp="currentValueTimestamp">
        <template v-slot:header>Indoor temperature</template>
...
export default {
    ...
    methods: {
        ...
        async fetchData() {
            ...
            const response = await axios.get("http://localhost/silver/value_timestamp?device_name=f1255pc&observation_name=room_temperature&observed_at_lower_bound=" + observedAtLowerBound)
