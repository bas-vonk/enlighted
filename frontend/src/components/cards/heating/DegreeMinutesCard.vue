<template>
    <base-card :observedAtTimestamp="currentValueTimestamp" :isMeasuredReliably="isMeasuredReliably">
        <template v-slot:header>Degree minutes</template>
...
export default {
    ...
    methods: {
        ...
        async fetchData() {
            ...
            const observedAtLowerBound = Math.floor(Date.now() / 1000) - (60 * 60 * 24 * 1)

            const response = await axios.get("http://localhost/silver/value_timestamp?device_name=f1255pc&observation_name=degree_minutes&observed_at_lower_bound=" + observedAtLowerBound)
