...
export default {
    name: 'SupplyTemperaturesCard',
    components: { DoubleValueCard },
    data() {
        return {
            cardTitle: "Supply temperature",
            observedAtTimestamp: "",
            leftValue: null,
            leftUnit: "&#176;C",
            leftExplanation: "Actual",
            rightValue: null,
            rightUnit: "&#176;C",
            rightExplanation: "Calculated",
    ...
    methods: {
        ...
        async fetchData() {
            ...
            const response_return = await axios.get("http://localhost/silver/value_timestamp?device_name=f1255pc&observation_name=heat_medium_flow_calculated_temperature&limit=1");
