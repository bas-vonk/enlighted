...
from enlighted.oauth2.models import (AccessToken, AccessTokenRow, RefreshToken,
                                     RefreshTokenRow)
...
def get_valid_token(
    ...
    access_token_row: AccessTokenRow | None = AccessToken.get_most_recent(session)
    ...
    refresh_token_row: RefreshTokenRow | None = RefreshToken.get_most_recent(session)
    ...
    AccessToken.store_token(
        session, response["access_token"], response["expires_in"], 0.50
    )
    RefreshToken.store_token(session, response["refresh_token"], response["expires_in"])
